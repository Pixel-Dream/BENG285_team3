...
simulate_mutations_wrapper <- function(i) {
  ...
  tmp <- estimate_random_mutation_effects(cdna, trx_info_df$n_mutation[i]+trx_info_df$s_mutation[i], 
                                          num_rounds = 2000, dN = trx_info_df$n_mutation[i],dS = trx_info_df$s_mutation[i])
  # Return results
  return(tmp)
}
...
write.csv(trx_info_df[,3:9],"/volume/data/BENG285/TCGA_LUAD_permutation_dN_dS_2000.csv")
...
write.csv(trx_info_df[trx_info_df$p_adj<0.05,3:11],"/volume/data/BENG285/TCGA_LUAD_permutation_dN_dS_2000_subset.csv")
