...
class SelectionTester:
    ...
    def test_selection(self, genes: Dict[str, Any], observed_mutations: Dict[str, List], 
        ...
        for gene_name, gene in genes.items():
            if gene_name not in expected_counts:
                continue
            ...
            expected = {
                'synonymous': sum(expected_counts[gene_name]['synonymous'].values()),
                'missense': sum(expected_counts[gene_name]['missense'].values()),
                'nonsense': sum(expected_counts[gene_name]['nonsense'].values()),
                'splice_site': sum(expected_counts[gene_name].get('splice_site', {}).values())
            }
